...
namespace pwn {
  ...
  public:
    postfix_writer(std::shared_ptr<cdk::compiler> compiler, cdk::symbol_table<pwn::symbol> &symtab,
        ...
        basic_ast_visitor(compiler), _symtab(symtab), _pf(pf), _lbl(0), _head(0), _level(0),
        _dataPointer(32768), _codePointer(0){
    }
  ...
  private:
    ...
    inline int dataPointer(){
      return _dataPointer;
    }
    inline void dataPointer(int value){
      _dataPointer += value;
    }
    inline int codePointer(){
      return _codePointer;
    }
    inline void codePointer(int value){
      _codePointer += value;
    }
